...
public class CommonClass {
    ...
    public static InteractionResultHolder<ItemStack> bombard(Player p, Level l, InteractionHand h, int bombingCD, int fuseTime, boolean inertia) {
        final ItemStack stack = p.getItemInHand(h);
        if (l.isClientSide()) return InteractionResultHolder.pass(stack);
        if (!p.isFallFlying()) return InteractionResultHolder.pass(stack);
        if (!stack.is(Items.FLINT_AND_STEEL) && !stack.is(Items.FIRE_CHARGE)) return InteractionResultHolder.pass(stack);
        final Item tnt4cd = Items.TNT.asItem();
        ...
        if (stack.isDamageableItem()) stack.hurtAndBreak(
                1, p,
                h == InteractionHand.MAIN_HAND ? EquipmentSlot.MAINHAND : EquipmentSlot.OFFHAND
        );
